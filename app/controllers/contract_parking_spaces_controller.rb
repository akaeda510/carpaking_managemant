class ContractParkingSpacesController < ApplicationController
  ...
  def create
    @contract_parking_space = @contractor.contract_parking_spaces.build(contract_params)
    # 契約者の登録管理者IDを記録
  ...
  def update
    authorize @contract_parking_space
    if @contract_parking_space.update(contract_params)
      notice_msg = params[:immediate_termination] == "1" ? "契約終了(解約)しました。" : "契約内容を更新しました。"
      redirect_to contractor_contract_parking_spaces_path(@contractor),
        notice: notice_msg,
        status: :see_other
    else
  ...
  def  set_contract_parking_space
